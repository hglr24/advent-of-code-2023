...
private fun classifyHand(handString: String): HandType {
    val distinctCards = handString.toCharArray().distinct()
    return when (distinctCards.size) {
        1 -> HandType.FIVE_OF_A_KIND
        2 -> {
            if (distinctCards.any { Regex(it.toString()).findAll(handString).count() == 4 }) {
                return HandType.FOUR_OF_A_KIND
            }
            return HandType.FULL_HOUSE
        }
        3 -> {
            if (distinctCards.any { Regex(it.toString()).findAll(handString).count() == 3 }) {
